...
const AdminPage: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      const res = await axios.get("https://novasysweb.onrender.com/newsEn");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("https://novasysweb.onrender.com/newsEn", formData, {
  ...
  const deleteNews = async (id: number) => {
    ...
    try {
      await axios.delete(`https://novasysweb.onrender.com/newsEn/${id}`);
      alert("Устгагдлаа");
      fetchNews(); // refresh
    } catch (err) {
      console.error(err);
          ...
          {newsList.map((news) => (
            ...
            >
              <div className="flex gap-4">
                {news.image_url && (
                  <img
                    src={`https://novasysweb.onrender.com/uploadsEn/${news.image_url}`}
                    alt={news.title}
