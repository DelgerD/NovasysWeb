...
const AdminPage: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      const res = await axios.get("http://localhost:8000/newsEn");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("http://localhost:8000/newsEn", formData, {
  ...
  const deleteNews = async (id: number) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/newsEn/${id}`);
      alert("Устгагдлаа");
      fetchNews(); // refresh
    } catch (err) {
      console.error(err);
          ...
          {newsList.map((news) => (
            ...
            >
              <div className="flex gap-4">
                {news.image_url && (
                  <img
                    src={`http://localhost:8000/uploadsEn/${news.image_url}`}
                    alt={news.title}
