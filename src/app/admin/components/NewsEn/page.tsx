...
const AdminPage: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      const res = await axios.get("http://https://novasysweb.onrender.com/newsEn");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("http://https://novasysweb.onrender.com/newsEn", formData, {
  ...
  const deleteNews = async (id: number) => {
    ...
    try {
      await axios.delete(`http://https://novasysweb.onrender.com/newsEn/${id}`);
      alert("Устгагдлаа");
      fetchNews(); // refresh
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-4xl text-black mx-auto py-10">
      ...
      {/* ==================== ADD NEWS FORM ==================== */}
      <h1 className="text-3xl font-bold mb-6">Мэдээ нэмэх англи</h1>
          ...
          {newsList.map((news) => (
            ...
            >
              <div className="flex gap-4">
                {news.image_url && (
                  <img
                    src={`http://https://novasysweb.onrender.com/uploadsEn/${news.image_url}`}
                    alt={news.title}
