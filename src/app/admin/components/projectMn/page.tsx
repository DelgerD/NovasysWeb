...
const AdminPage: React.FC = () => {
  ...
  const fetchProject = async () => {
    try {
      const res = await axios.get("http://localhost:8000/projectMn");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("http://localhost:8000/projectMn", formData, {
  ...
  const deleteProject = async (id: number) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/projectMn/${id}`);
      alert("Устгагдлаа");
          ...
          {ProjectList.map((Project) => (
            ...
            >
              <div className="flex gap-4">
                {Project.image_url && (
                  <img
                    src={`http://localhost:8000/uploadsMn/project/${Project.image_url}`}
                    alt={Project.title}
